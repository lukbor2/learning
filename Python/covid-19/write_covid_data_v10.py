...
TODO 6: add details for Switzerland (https://github.com/daenuprobst/covid19-cases-switzerland)
    ...
    Think how to update the canton code with the canton name.
    ...
    with open(file_us_details, newline='') as csvfile:
        ...
        for row in reader:
            if row[headers[1]] in state_list_01:
                ...
                if tested =='':
                    tested = '0'
                parameters = (tested, row[headers[2]], hospitalized, in_icu, state_name, date(int(s[:4]),int(s[4:6]),int(s[6:8])).isoformat())
                #print(parameters)
...
def add_swiss_details():
    #The idea is to do a first INSERT with total cases directly in final_01; then use UPDATE for all other data.

    swiss_total = '/home/luca/git/COVID-19_switzerland/covid19-cases-switzerland/covid19_cases_switzerland_openzh.csv'
    add_ch = ("INSERT INTO final_01 "
               "(final_01_country, final_01_region, final_01_date, confirmed_number) "
               "VALUES (%s, %s, %s, %s) "
               )
