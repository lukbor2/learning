...
TODO: add one function which sets to 0 all data in final_01 for the US total country execept the fields which holds recovered and current positive.
If I do this I can still have the current positive for the US as total.

TODO: add the data about Regions in Italy.
    ...
    with open(file_us_details, newline='') as csvfile:
        ...
        for row in reader:
            if row[headers[1]] in state_list_01:
                ...
                if in_icu =='':
                    in_icu = '0'
                parameters = (row[headers[18]], row[headers[2]], hospitalized, in_icu, state_name, date(int(s[:4]),int(s[4:6]),int(s[6:8])).isoformat())
                #print(parameters)
                #NOTE: on 5/5/2020 it seems the file structure changed and the tested data moved to column 18 from 16. I changed the line above with the parameters.
