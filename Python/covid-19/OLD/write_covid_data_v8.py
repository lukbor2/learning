...
def add_italy_regions():
    ...
    for my_file, my_qry in zip(file_names, queries_list):
        with open(my_file, newline='') as csvfile:
            ...
            headers = reader.fieldnames
            for row in reader:
                parameters = ('Italy', row[headers[3]], row[headers[0]][:10] + ' 00:00:00' , row[headers[15]], row[headers[14]], row[headers[13]], row[headers[10]], row[headers[6]], row[headers[7]], row[headers[8]], row[headers[16]], row[headers[15]],)
                #print(parameters)
                cursor.execute(my_qry, parameters)
    mydb.commit()
