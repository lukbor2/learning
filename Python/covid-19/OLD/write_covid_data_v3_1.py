...
delete_tables = []
delete_tables.extend(["delete from confirmed","delete from deaths", "delete from recovered", "delete from final_01", "delete from us_states"])
...
def add_us_states():
    qry = ("INSERT INTO us_states "
           "(us_states_date, us_states_state, us_states_fips, us_states_cases, us_states_deaths)"
           " VALUES (%s,%s,%s,%s,%s)"
           )
           #"(us_states_date,us_states_state, us_states_fips, us_states_cases, us_states_deaths,us_states_cases_delta, us_states_cases_delta_p, us_states_deaths_delta,us_states_deaths_delta_p)"
    ...
    qry_states_list = ("SELECT DISTINCT us_states_state from us_states ORDER BY us_states_state") # This is the list of the states.
    qry_data = ("SELECT * from us_states WHERE us_states_state = %s ORDER BY us_states_state, us_states_date")
    qry_update = ("UPDATE us_states "
             "SET us_states_cases_delta = %s, us_states_cases_delta_p = %s, us_states_deaths_delta = %s, us_states_deaths_delta_p = %s "
             "WHERE us_states_state = %s AND us_states_date = %s"
    )
    ...
    for row in cursor_states:
        ...
        for data in records:
            ...
            if (i+1) < len(records):
                ...
                parameters = (cases_delta, cases_delta_p, deaths_delta, deaths_delta_p, data_next[2], data_next[1])
