...
def make_graph(country_1, country_2, region_1, region_2):
    ds_country_1 = ds[(ds["final_01_country"] == country_1) & (ds["final_01_region"] == region_1)]
    ds_country_2 = ds[(ds["final_01_country"] == country_2) & (ds["final_01_region"] == region_2)]
    #print(ds_country_1.head())
    measure_1 = "current_positive"
    measure_2 = "current_positive_delta_p"
    #print(ds_country_1[measure_1])
...
def set_region_list(country_1):
    qry = 'SELECT DISTINCT final_01_region FROM final_01 WHERE final_01_country = \'' + country_1 + '\' ORDER BY final_01_region'
    ds_regions = pd.read_sql(qry, con=db_connection)
    ds_regions = ds_regions.append({'final_01_region': '""'}, ignore_index=True) #I have to appen this value because it selects the entire country.
    return([{"label": col, "value": col} for col in ds_regions["final_01_region"]])
